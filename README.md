...
核心发现：**在BTC市场上，复杂度和收益不成正比——简单规则优于机器学习，而买入持有打败了所有主动策略。**
...
graph TD
    A["🗂 NB1: 数据获取与清洗"]:::data
    A -->|"35,064条K线<br>完整度99.96%"| B["📈 NB2: 经典技术指标"]:::strat
    B -->|"全部亏损<br>止损越加越差"| C["📊 NB3: 统计驱动策略"]:::strat
    C -->|"Z-score首次正收益<br>+35%但跑不赢持有"| D["🤖 NB4: 机器学习信号"]:::strat
    D -->|"过拟合<br>不如简单规则"| E["⚙️ NB5: 回测引擎"]:::engine
    E -->|"加入滑点<br>收益缩水一半"| F["📉 NB6: 绩效分析"]:::analysis
...
```mermaid
graph LR
    A["🟢 低复杂度<br>买入持有<br>夏普 0.54"]:::good --> B["🟡 中复杂度<br>Z-score均值回归<br>夏普 0.10~0.19"]:::mid
    B --> C["🔴 高复杂度<br>XGBoost<br>夏普 -1.29"]:::bad
...
> 数据含0.1%手续费 + 0.05%滑点，NB4仅测试集（2023.10-2024.12）

### 3. 三个反直觉的结论

- **止损不一定保护你**：在BTC的高波动下，固定止损和移动止损都因频繁触发反而加大了亏损（NB2详细验证）
- **交易频率是隐性杀手**：同一策略框架下，交易21次（夏普0.19）远好于交易143次（夏普0.10）
- **ML不如简单规则**：XGBoost学到的最重要特征（ma_ratio_24, zscore_168）和手动规则完全一致，但它无法比规则做得更好
...
```bash
# 克隆仓库
git clone https://github.com/你的用户名/crypto-trading-research.git
